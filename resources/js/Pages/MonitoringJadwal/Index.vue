...
const tipeLabel = (tipe) => (tipe === 'online' ? 'ONLINE' : 'OFFLINE');
const tipeColor = (tipe) => (tipe === 'online' ? 'text-blue-600' : 'text-gray-600');
...
const hasActiveFilter = () =>
    selectedStatus.value !== 'all' || searchQuery.value !== '';
</script>
                    ...
                    <div class="p-4">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                            ...
                            <div class="flex items-center gap-2">
                                <label for="filter-status" class="flex items-center gap-1.5 text-sm font-medium text-gray-600 whitespace-nowrap">
                                    <svg class="h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <div v-if="hasActiveFilter()" class="mt-2 flex flex-wrap gap-2">
                            <span v-if="selectedStatus !== 'all'" class="inline-flex items-center gap-1 rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-medium text-indigo-800">
                                Status: {{ statusOptions.find(o => o.value === selectedStatus)?.label }}
                            </span>
                            <span v-if="searchQuery" class="inline-flex items-center gap-1 rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-medium text-indigo-800">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                ...
                                <tr v-if="jadwalBimbingans.length === 0">
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        ...
                                        <p class="mt-2 text-sm font-medium text-gray-500">Tidak ada jadwal bimbingan ditemukan.</p>
                                        <p v-if="hasActiveFilter()" class="mt-1 text-xs text-gray-400">
